...
router.get('/per-week', async (req, res) => {
  try {
    const result = await db('event')
...
router.get('/per-month', async (req, res) => {
  try {
    const result = await db('event')
...
router.get('/top-users', async (req, res) => {
  try {
    const result = await db('event')
      ...
      .limit(10);
    // Optionally join with user table for usernames
...
router.get('/avg-time-between', async (req, res) => {
  try {
    // Get all users
    ...
    for (const user of users) {
      ...
      for (let i = 1; i < events.length; i++) {
        ...
        totalDiff += (curr - prev) / (1000 * 60 * 60); // ms to hours
      }
